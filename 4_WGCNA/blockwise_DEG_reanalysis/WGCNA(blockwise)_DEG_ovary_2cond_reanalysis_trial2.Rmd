...
```{r}
cpmlog = read.csv(here("4_WGCNA/data_input/ovary_cpmLog_filterByExpr_20240702.csv"))
cpmlog[1:5,1:5]
```
...
timecourse_deg = read_excel(here("3_DEG/edgeR_timecourse_reanalysis/ovary_trial2/cond16/results/ovary_cond16_sig_timecourseGenes_cubicRegDF3.xlsx"))
head(timecourse_deg)
```
...
if (!gsg$allOK) {
  ...
  expr_sig <- expr_sig[gsg_ovary_CV75$goodSamples, gsg_ovary_CV75$goodGenes]
}
...
```{r check-outlier-sampels-ovary,fig.width=15, fig.height=6}
# cluster the samples by clusters
sampleTree <- hclust(dist(expr_sig), method = "average")

pdf(file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/SampleClustering_ovary.pdf"), width = 15, height = 6)
par(cex = 1)
par(mar = c(0, 5, 2, 0))
plot(sampleTree,
  main = "Sample clustering to detect outliers - ovary",
  sub = "",
  xlab = "",
  cex.lab = 1,
  cex.axis = 1,
...
pdf(file=here::here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/ovary_hclusterTree.pdf"), onefile=F, paper="special", width=15, height=8, bg="white", pointsize=12)
...
png(file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/sft_ovary.png"), width = 9, height = 4.5, units = "in", res = 300)
par(mfrow = c(1,2));
par(cex = 0.8);
cex1 = 0.9
...
plot(sft$fitIndices[, 1],
     ...
     type = "n",
     main = paste("Scale independence - ovary"))
...
plot(sft$fitIndices[, 1],
     ...
     type = "n",
     main = paste("Mean connectivity - ovary"))
text(sft$fitIndices[, 1],
     sft$fitIndices[, 5],
     labels = powers,
     cex = cex1, col = "red")
...
net = blockwiseModules(expr_sig, 
                       ...
                       ## ==save TOM matrix==##
                       # saveTOMs=TRUE,  
                       # saveTOMFileBase = here::here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/TOM_ovary_2cond"),
                       # loadTOM = TRUE,
...
save(
   net,
   file = here::here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/network_ovary_2cond.RData"
 ))
...
plotDendroAndColors(net$dendrograms[[1]],
                    ...
                    main = "Cluster Dendrogram - ovary 2cond"
                    )
```

```{r plot-dendrogram-after-merged-only, fig.width=10, fig.height=6}
pdf(file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/geneTree_merged_ovary_2cond.pdf"), width = 10, height = 6,pointsize=20)
# png(file = here("4_WGCNA/m2_inputData_organSeparate_CV50/ovary/plots/geneTree_merged_ovary_2cond_cv50.png"), width = 12, height = 7,units = "in",res = 300) 
...
png(file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/beforeMerge_moduleGenes_ovary_2cond.png"), width = 20, height = 6, units = "in",res = 300)
par(cex = 1)
par(mar = c(0, 5, 2, 0))
...
plot_moduleSum_unmerged = module_sum_beforeMerge %>% 
  ...
  labs(x = "Module ID", fill = "Module Name (before merge)", title = "Module genes summary (before merge) - ovary,2cond") +
...
png(file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/afterMerge_moduleGenes_ovary_2cond.png"), width = 14, height = 6, units = "in",res = 300)
par(cex = 1)
par(mar = c(0, 5, 2, 0))
...
plot_moduleSum_merged = module_sum_afterMerge %>% 
  ...
  labs(x = "Module ID", fill = "Module Name (after merge)", title = "Module genes summary (after merge) - ovary,2cond") +
  geom_text(aes(label = `No. of genes`), vjust = -0.5,angle = 45,hjust = -0.25)+
  # make legends to three columns
...
# save it
write.csv(eig_mean, file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/MEs_mean_ovary_2cond.csv"),row.names = F)
```
...
custom_labels1=c("MEturquoise" = "MEturquoise (1633)","MEblue" = "MEblue (1288)","MEbrown" = "MEbrown (1168)","MEyellow" = "MEyellow (1057)","MEgreen" = "MEgreen (795)","MEred" = "MEred (662)","MEblack" = "MEblack (510)","MEpink" = "MEpink (421)","MEmagenta" = "MEmagenta (365)","MEpurple" = "MEpurple (292)","MEgreenyellow" = "MEgreenyellow (215)","MEtan" = "MEtan (212)","MEsalmon" = "MEsalmon (182)","MEcyan" = "MEcyan (169)","MEmidnightblue" = "MEmidnightblue (160)","MElightcyan" = "MElightcyan (145)","MEgrey60" = "MEgrey60 (95)","MElightgreen" = "MElightgreen (94)","MElightyellow" = "MElightyellow (90)","MEroyalblue" = "MEroyalblue (84)","MEdarkgreen" = "MEdarkgreen (77)","MEdarkred" = "MEdarkred (77)","MEdarkturquoise" = "MEdarkturquoise (57)","MEdarkgrey" = "MEdarkgrey (53)")
```
...
eig_lineplot <- eig_mean %>% 
  ...
  facet_wrap(vars(module),scales = "free_x",ncol = 6,labeller = labeller(module = custom_labels1)) +
  # scale_color_manual(values = rank_moduleColors_ovary[rank_moduleColors_ovary != "grey"]) +
  labs(x="", y="Normalized eigengene expression", 
       title = "Module eigengene expression - ovary 2cond") +
  theme_bw() +
...
# save the plot
ggsave(eig_lineplot, 
       filename = here::here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/Merge_MEs_avgExpr_ovary_2cond_yfixed_reanalysis.png"),
...
save(
   ...
   eig_mean,
   eig_mean,
   file = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results//WGCNA_ovary_2cond_reanalysis.RData"
 ))
```
...
clus_plot1 = clus_sum2 %>% 
  ...
  labs(x = "ZT", y = "Normalized expression", title = "WGCNA of DEGs across time: ovary 2cond")+
  # scale_color_manual(
...
png(here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/plots/ovary_2cond_normalized_gene_expression.png"), width = 18, height=12, units = "in",res = 300)
print(clus_plot1)
dev.off()
...
# write.csv(modules.after, here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/merged_ovary_2cond_moduleAssignment.csv"))
```
...
```
I tried with the DEGs of cond16, namely 20920 genes for ovary.
```
...
for (i in mod){
  ...
  GO.sig.DEG[[i]] = GO.sig
  # names(GO.sig.ovary.afterMerge[[i]]) = i
}

writexl::write_xlsx(x = GO.enrich.DEG, path = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.allEnrichIterm.ovary.2cond.ALLbg.re.xlsx"))

writexl::write_xlsx(x = GO.sig.DEG, path = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.sig.ovary.2cond.ALLbg.re.xlsx"))
```

```{r}
go.allEnrich = do.call(rbind, GO.enrich.DEG) %>% 
  as.data.frame() %>% 
  mutate(organ = paste("ovary"), condition = paste("14&16"), merge = paste("after")) 
writexl::write_xlsx(x = go.allEnrich, path = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.allEnrichIterm.ovary.2cond.allModules.ALLbg.re.xlsx"))

go.sig = do.call(rbind, GO.sig.DEG) %>% 
  as.data.frame() %>% 
  mutate(organ = paste("ovary"), condition = paste("14&16"), merge = paste("after")) 
writexl::write_xlsx(x = go.sig, path = here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.sig.ovary.2cond.allModules.ALLbg.re.xlsx"))
```
...
```{r}
GO.sig.genes = readxl::read_excel(here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.sig.ovary.2cond.allModules.ALLbg.re.xlsx"))
head(GO.sig.genes)
```
...
writexl::write_xlsx(GO.sig.EnrichmentFold, here("4_WGCNA/blockwise_DEG_reanalysis/ovary_2cond_trial2/results/GO.sig.ovary.2cond.allModules.FoldEnrichment.ALLbg.re.xlsx") )
```
